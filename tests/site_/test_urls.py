...
def test_the_cross_act_date_list_sits_one_directory_down() -> None:
    """A function rather than a literal, for the reason `amendments_href` is one: the builder,
    the header bar and the sitemap all need the string and must not spell it three ways."""
    assert dates_href() == "dates/"
    assert depth_of(dates_href()) == 1
