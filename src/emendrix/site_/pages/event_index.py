...
- **It is a grid of coordinates until there is room for a column.** Forty-five provisions cost
  a few lines of height as a grid and a screen of scrolling as a column, so the grid, each
  coordinate marked by its kind's glyph, is what a phone gets; the stylesheet turns it into the
  sticky column at the width the act page's own index becomes one, and the count line is the
  label the column needs when it stands on its own.
...
__all__ = ["INDEX_ABOVE", "INDEX_ID", "touched"]
...
INDEX_ID = "changes-index"
"""The index's `id`: what the context bar and every change's way back link to."""
...
def touched(
    ...
    lines = [
        Html(f'<nav class="touched" id="{INDEX_ID}" aria-label="Changes in this version">'),
        Html(
