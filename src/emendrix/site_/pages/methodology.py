...
def _how_this_site_is_built(site: SiteInputs) -> list[Html]:
    ...
    return [
        ...
        Html(
            f"<p>A directory of static files, generated by <code>emendrix site build</code> from "
            f"artifacts committed in {source} and in a changelog repository the operator owns: "
            f"the changelog documents this tool wrote and the dated evaluation report it was "
            f"scored against. Where that repository lives is deliberately not printed here.</p>"
        ),
        Html(
            ...
            "clock, reaches the network, or calls a model. One small script adds search; the "
            "only links that leave the site go to EUR-Lex.</p>"
        ),
    ]
