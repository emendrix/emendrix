...
/* A short version's changes, named first so a phone's first screen says what changed. The
   lead is set as a caption, the coordinates are the links, and a long title wraps. */
.changes-line { font-size: var(--text-body); line-height: 1.5; max-width: 80ch;
                margin-bottom: var(--space-2); overflow-wrap: anywhere; }
.changes-line p { margin: 0; }
.changes-line .lead { font: 600 var(--text-caption)/1.3 var(--sans); color: var(--fg);
                      margin-right: .15rem; }
.changes-line a { font-weight: 600; }
.changes-line .ttl { color: var(--fg); }
