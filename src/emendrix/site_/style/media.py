...
a one-class one whatever the order, so `.chg:target`, the two tinted verbatim blocks and the
filled alert badge are named again rather than left to the blanket line. The block takes the
header's navigation, the search mount and the pager off the page, drops the background tints
that cost ink and say nothing on paper, and keeps the disclaimer, which every user-facing
output of this project carries. Paper is white and ink is black, the one place the sheet names
a colour that is not a token, because a printed page has no scheme to follow.
...
@media print {
  ...
  .strip, .disclaimer, .sidebar, .touched, .diff, .verbatim, .lbl, .loop li, .pill, .tag
  ...
  /* Named again because a two-class or pseudo-class selector outranks a one-class one
     ...
     above a whole inserted or deleted provision, in the word inside the pill, and in the
